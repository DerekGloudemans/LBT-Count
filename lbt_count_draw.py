...
class LBT_Count():
    ...
    def __init__(self,
        ...
        self.running_avg_frame = None
        self.running_avg_count = 0
        
        self.movement_colors = np.random.rand(13,3)*255
        self.movement_colors[0:6,:] = np.array([[255,0,255],
                                         [255,255,0],
                                         [0,255,255],
                                         [255,100,0],
                                         [0,255,100],
        ...
        self.movement_colors = [(int(item[0]),int(item[1]),int(item[2])) for item in self.movement_colors]
    ...
    def track(self):
        ...
        while frame_num != -1: 
            ...
            if frame is not None:
                try:
                    self.running_avg_frame += frame.data.cpu()
            ...
            fps_noload = round(frame_num/(time.time()-self.start_time-self.time_metrics["load"] - self.time_metrics["plot"]),1)
